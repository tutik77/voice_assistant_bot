...
async def convert_text_to_voice(response: str):
...
async def handle_voice_message(message: types.Message):
    ...
    response = "у тебя все плохо и тебя не любит мама"
    ...
    voice = await convert_text_to_voice(response)
    await bot.send_voice(message.chat.id, voice)

@dp.message(F.photo)
async def handle_photo(message: types.Message):
    path_to_photo = f'{DIRECTORY}/photo.jpg'
    if message.photo:
